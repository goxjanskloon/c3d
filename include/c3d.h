...
namespace c3d{
    ...
    public:
        ...
        [[nodiscard]] bool hit(const vector &origin,const vector &ray,interval interval)const{
            return !(interval.intersect({(x.min-origin.x)/ray.x,(x.max-origin.x)/ray.x}).empty()
                   ||interval.intersect({(y.min-origin.y)/ray.y,(y.max-origin.y)/ray.y}).empty()
                   ||interval.intersect({(z.min-origin.z)/ray.z,(z.max-origin.z)/ray.z}).empty());
        }
    ...
    public:
        virtual ~hittable()=0;
        [[nodiscard]] virtual std::shared_ptr<hit_record> hit(const vector& origin,const vector& ray,const interval& interval)const=0;
    ...
    public:
        ...
        [[nodiscard]] virtual std::shared_ptr<scatter_record> scatter(const ray& in,const hit_record &hit)const{
            return nullptr;
        }
    };
    class bvh_tree final:public hittable{
        bvh_tree(std::vector<std::shared_ptr<const hittable>> &objects,const int l,const int r){
            if(const std::size_t n=objects.size();n==1){
                ...
                std::sort(std::execution::par_unseq,lp,rp,[&](const std::shared_ptr<const hittable> &a,const std::shared_ptr<const hittable> &b){
                    if(xl>yl&&xl>zl)
    ...
    public:
        ...
        [[nodiscard]] std::shared_ptr<hit_record> hit(const vector &origin,const vector &ray,const interval &interval)const override{
