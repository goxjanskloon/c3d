...
void renderer3d::render(const vector3d &pos,const vector3d &facing,const vector3d &ud,const vector3d &rd,const int &width,const int &height,const PIMAGE &pimg){
    ...
    auto f=[&](const int &lx,const int &rx,const int &ly,const int &ry){
    ...
    std::async(std::launch::async,f,0,hw,0,hh),
    std::async(std::launch::async,f,hw,width,0,hh),
    std::async(std::launch::async,f,0,hw,hh,height),
    std::async(std::launch::async,f,hw,width,hh,height);
}
