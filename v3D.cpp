...
void renderer3d::render(const vector3d &pos,const vector3d &facing,const vector3d &ud,const vector3d &rd,const int &width,const int &height,const PIMAGE &pimg){
    ...
    auto f=[](const int &i,const int &j,const renderer3d &renderer,const vector3d &pos,const vector3d &facing,const vector3d &ud,const vector3d &rd,const int &hh,const int &hw,const PIMAGE &pimg){
        std::list<std::pair<double,color_t>> px;
        for(auto &fp:renderer){
            auto &f=*fp;
    ...
    for(int i=0;i<height;i++)
        for(int j=0;j<width;j++) std::async(std::launch::async|std::launch::deferred,f,i,j,*this,pos,facing,ud,rd,hh,hw,pimg);
}
